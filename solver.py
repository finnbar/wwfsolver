...
def prettyPrint(grid):
	'''
	Print out a Grid in a neat format.
	'''
	for i in range(15):
		s = ""
		for j in range(15):
...
def placeOnGrid(grid,placements,word):
	'''
	Takes a word (a string) and a list of placements (generated by findPlacements), and places that on a copy of the grid.
	'''
	# p is placements
	g = copy.deepcopy(grid)
	for i in range(len(placements)):
		g[placements[i][0]][placements[i][1]] = word[i]
	return g

def findPlacements(grid,solution):
	'''
	Takes a solution (a tuple of (word,row,column,orientation,positionsOfBlanks)) and works out its "placements" - where each tile in the solution should go.
	'''
	placements = []
	w,sr,sc = solution[0],solution[1],solution[2]
	if solution[3] == "down":
		for i in range(sr-len(w)+1,sr+1):
			grid[i][sc] = w[i+len(w)-1-sr]
	else:
		for i in range(sc-len(w)+1,sc+1):
			grid[sr][i] = w[i+len(w)-1-sc]
	return placements

def findAllWords(g,line,tiles,row,column,word,flipped,dictionary,blanks=[],attached=False,usedTile=False):
	'''
	The main word-finding function. It tries every possible placement for that square, then checks if it could be part of a word, and then adds it to the word list if its complete, and calls this function again to work out what to place next.
	'''
	WordList = []
...
def doRows(grid,tiles,dictionary,flipped=False):
	'''
	Finds the rows in the grid, marking each square with the possible tiles that it could be, ready to be used by findAllWords.
	'''
	WordList = []
...
def doColumns(grid,tiles,dictionary):
	'''
	Flips the grid and then does doRows to essentially doColumns.
	'''
...
def scoreMove(word,grid,modifiers,verbose=False):
	'''
	This takes a solution, and scores it by splitting it into its placements and scoring each one.
	'''
...
def wordMultiplier(score,wordModifiers):
	'''
	A helper function for scoreMove, which takes a list of modifiers that affect the whole word (double word or triple word) and multiplies the score based on that.
	'''
...
def scoreLetter(tile,grid,modifiers,wordModifiers,verbose):
	'''
	A helper function for scoreMove, which scores a tile based on its value and the modifier there.
	'''
